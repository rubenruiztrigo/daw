...
export const StoreView: React.FC<StoreViewProps> = ({ user, language, onRedeemReward, storeRewards = [], userRedemptions = [] }) => {
    ...
    return (
        <div className="w-full px-4 md:px-8 pt-20 md:pt-6 space-y-6 animate-in fade-in duration-500">
            <div className="flex items-center justify-between mb-8 px-1">
                <h2 className="text-2xl font-black text-slate-900 dark:text-white">Recompensas</h2>
                <div className="flex items-center bg-purple-600 dark:bg-purple-700 text-white px-4 py-1.5 rounded-xl shadow-lg border border-purple-500/30 transition-transform hover:scale-105">
                    <div className="flex items-baseline space-x-1.5">
                        <span className="font-black text-base tracking-tight">{points}</span>
                        <span className="text-[8px] font-black uppercase tracking-widest opacity-90">Novas</span>
                    </div>
                </div>
            </div>
                        ...
                        <div key={reward.id} className={`group relative bg-white dark:bg-[#111] rounded-[2rem] md:rounded-[2.5rem] border transition-all duration-500 p-4 md:p-6 flex flex-col items-center text-center ${isUnlocked ? 'border-slate-100 dark:border-zinc-800 hover:border-purple-500/50 hover:shadow-2xl hover:shadow-purple-500/10 hover:-translate-y-2' : 'border-slate-100 dark:border-zinc-800 opacity-75'}`}>
                            ...
                            <div className="mt-auto w-full">
                                {isRedeemed ? (
                                    ...
                                    <div className="flex items-center justify-center space-x-2 text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest shadow-inner">
                                    ...
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-center space-x-2 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest transition-all ${isProcessing ? 'bg-slate-100 dark:bg-zinc-800 text-slate-400' : 'bg-emerald-500 text-white hover:bg-emerald-600 shadow-lg shadow-emerald-500/20'}`}
                                    >
