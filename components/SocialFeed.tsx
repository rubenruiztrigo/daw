...
import { ImageIcon, Clapperboard, Smile, X, Users, Sparkles, Plus, AtSign } from 'lucide-react';
...
export const SocialFeed: React.FC<SocialFeedProps> = ({ 
  posts, user, onLike, onVote, onRepost, onAddPost, onAddComment, onDeletePost, onSearchHashtag, onSharePost, onNavigateToProfile, onNavigateToPost, followedUserIds = new Set(), followerUserIds = new Set(), onToggleFollow, users = [],
}) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAddPost(content, 'post', tags, selectedImage || undefined, selectedDoc?.url, selectedDoc?.name);
    setContent(''); setSelectedImage(null); setSelectedDoc(null);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4 pb-20">
      ...
      <div className="bg-white dark:bg-[#111] p-5 rounded-2xl border border-gray-100 dark:border-zinc-800 shadow-sm relative">
        <div className="flex space-x-4">
          ...
          <form onSubmit={handleSubmit} className="flex-1">
            ...
            {mentionQuery !== null && mentionSuggestions.length > 0 && (
              <div className="absolute left-16 mt-0 w-64 bg-white dark:bg-[#1a1a1a] rounded-2xl shadow-2xl border border-gray-100 dark:border-zinc-800 z-[60] overflow-hidden animate-in fade-in zoom-in-95 duration-100">
            ...
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-50 dark:border-zinc-800">
              <div className="flex items-center space-x-1">
                ...
                <button type="button" className="p-2 text-blue-500 hover:bg-blue-50 dark:hover:bg-zinc-800 rounded-full transition-colors opacity-50 cursor-not-allowed"><Clapperboard size={20} /></button>
              ...
              <button type="submit" disabled={!content.trim() && !selectedImage && !selectedDoc} className="bg-blue-600 text-white px-6 py-2 rounded-full font-bold text-sm disabled:opacity-50 transition-all hover:bg-blue-700 shadow-md">Publicar</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filteredPosts.length > 0 ? (
          ...
          <div className="text-center py-20 bg-white dark:bg-[#111] rounded-[2.5rem] border border-dashed border-slate-200 dark:border-zinc-800 px-10">
            ...
            {activeTab === 'for-you' && (
              <button onClick={() => onNavigateToProfile?.(user.id)} className="inline-flex items-center space-x-2 px-6 py-3 bg-blue-600 text-white rounded-2xl font-black text-xs hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 dark:shadow-none"><Plus size={16} /><span>Configurar mis intereses</span></button>
            )}
          </div>
        )}
      </div>
