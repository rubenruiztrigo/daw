...
import { Briefcase, MapPin, Share2, Edit3, Calendar, LayoutGrid, Newspaper, UserPlus, UserMinus, Camera, MessageCircle, Plus, Award } from 'lucide-react';
...
export const ProfileView: React.FC<ProfileViewProps> = ({ 
  ...
  onUpdateUser, onDeletePost, onNavigateToProfile, currentUser, onLike, onVote, onAddComment, users, onSearchHashtag
}) => {
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-12 animate-in fade-in duration-500">
      ...
      <div className="bg-white dark:bg-[#111] rounded-[2.5rem] overflow-hidden border border-gray-100 dark:border-zinc-800">
        ...
        >
          ...
          {isCurrentUser && (
            <button className="absolute bottom-4 right-6 p-2.5 bg-white/20 backdrop-blur-md text-white rounded-xl hover:bg-white/30 transition-all border border-white/20">
              <Camera size={20} />
            </button>
          )}
        </div>
        <div className="px-10 pb-10">
          <div className="relative flex flex-col md:flex-row justify-between items-start md:items-end -mt-12 mb-8 gap-6">
            <div className="relative flex-shrink-0">
              <img 
                src={user.avatar} 
                className="w-44 h-44 rounded-[2.5rem] border-8 border-white dark:border-zinc-800 object-cover transition-all cursor-pointer hover:opacity-90 active:scale-95" 
                alt="" 
      ...
      <div className="space-y-6">
        <div className="flex space-x-12 border-b border-gray-100 dark:border-zinc-900 px-6 overflow-x-auto scrollbar-hide">
          <button 
