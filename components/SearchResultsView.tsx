...
export const SearchResultsView: React.FC<SearchResultsViewProps> = ({
  ...
  const resultsPosts = useMemo(() => {
    const filtered = posts.filter(post => normalizeString(post.content).includes(normalizedQuery) || post.tags.some(tag => normalizeString(tag).includes(normalizedQuery)));
    return [...filtered].sort((a, b) => ((a.likes || 0) + (a.comments || 0)) - ((b.likes || 0) + (b.comments || 0)));
  }, [posts, normalizedQuery]);

  const peopleResults = useMemo(() => {
    if (!normalizedQuery) return users;
    return users.filter(u => normalizeString(u.name || '').includes(normalizedQuery) || normalizeString(u.lastName || '').includes(normalizedQuery) || normalizeString(u.position || '').includes(normalizedQuery) || normalizeString(u.department || '').includes(normalizedQuery));
  ...
  const uniqueHashtags = useMemo(() => {
    ...
    posts.forEach(post => post.tags.forEach(tag => {
      ...
      if (normalizeString(cleanTag).includes(normalizedQuery.replace(/\s+/g, ''))) tags.add(cleanTag);
    }));
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      ...
      <div className="flex bg-gray-100 dark:bg-zinc-900 p-1 rounded-2xl border border-gray-200 dark:border-zinc-800 w-fit">
        <button onClick={() => setActiveTab('featured')} className={`flex items-center space-x-2 px-6 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === 'featured' ? 'bg-white dark:bg-[#111] shadow-sm text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}><Zap size={14} /><span>Publicaciones</span></button>
        <button onClick={() => setActiveTab('people')} className={`flex items-center space-x-2 px-6 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === 'people' ? 'bg-white dark:bg-[#111] shadow-sm text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}><Users size={14} /><span>Personas ({peopleResults.length})</span></button>
        <button onClick={() => setActiveTab('hashtags')} className={`flex items-center space-x-2 px-6 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === 'hashtags' ? 'bg-white dark:bg-[#111] shadow-sm text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}><Hash size={14} /><span>Hashtags</span></button>
      </div>
      ...
      <div className="space-y-4">
              ...
              return (
                <div key={person.id} className="bg-white dark:bg-[#111] p-6 rounded-[2rem] border border-gray-100 dark:border-zinc-800 flex items-center justify-between hover:shadow-lg transition-all cursor-pointer group" onClick={() => onNavigateToProfile?.(person.id!)}>
                  ...
                  {person.id !== currentUser.id && (
                    <button onClick={(e) => { e.stopPropagation(); onToggleFollow?.(person.id!); }} className={`p-3 rounded-xl transition-all ${isMutual ? 'bg-green-50 dark:bg-green-900/20 text-green-600' : followedUserIds.has(person.id!) ? 'bg-slate-100 dark:bg-zinc-800 text-slate-400' : 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 hover:bg-blue-600 hover:text-white'}`}>
                      {isMutual ? <Check size={18} /> : followedUserIds.has(person.id!) ? <UserMinus size={18} /> : <UserPlus size={18} />}
                    </button>
                  )}
                </div>
              );
        ...
        {activeTab === 'hashtags' && (uniqueHashtags.length === 0 ? (
          <div className="text-center py-20 bg-white dark:bg-[#111] rounded-[32px] border border-dashed border-gray-200 dark:border-zinc-800"><Hash className="mx-auto text-gray-200 dark:text-zinc-800 mb-4" size={48} /><p className="text-gray-400 dark:text-zinc-600 font-bold italic">No hay hashtags.</p></div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {uniqueHashtags.map(tag => <button key={tag} onClick={() => onSearchHashtag?.(tag)} className="bg-white dark:bg-[#111] p-4 rounded-2xl border border-gray-100 dark:border-zinc-800 flex flex-col items-center justify-center space-y-2 hover:border-blue-500 hover:shadow-lg transition-all group"><div className="p-3 bg-blue-50 dark:bg-zinc-900 text-blue-600 rounded-xl group-hover:bg-blue-600 group-hover:text-white transition-all"><TrendingUp size={20} /></div><span className="text-sm font-black text-gray-900 dark:text-white">#{tag}</span></button>)}
          </div>
        ))}
      </div>
    </div>
...
const SimpleResultCard: React.FC<{ post: Post, onLike: (id: string) => void, onVote: (id: string, dir: 'up' | 'down') => void, onRepost: (id: string) => void, onAddComment: (postId: string, text: string) => void, onDeletePost?: (id: string) => void, onSearchHashtag?: (tag: string) => void, onShareViaChat?: (recipientId: string, text: string, sharedPostId?: string, sharedProfileId?: string) => void, onNavigateToProfile?: (userId: string) => void, onNavigateToPost?: (postId: string) => void, onPreviewImage?: (url: string) => void, currentUser: User, followedUserIds: Set<string>, followerUserIds: Set<string>, onToggleFollow?: (userId: string) => void, users: User[], onNavigateToEvent?: (userId: string, eventId: string) => void }> = ({ post, onLike, onVote, onRepost, onAddComment, onDeletePost, onSearchHashtag, onShareViaChat, onNavigateToProfile, onNavigateToPost, onPreviewImage, currentUser, followedUserIds, followerUserIds, onToggleFollow, users, onNavigateToEvent }) => {
  ...
  return (
    <div className={`bg-white dark:bg-[#111] rounded-[2rem] p-6 border transition-all cursor-pointer hover:border-gray-200 dark:hover:border-zinc-800 ${isNews ? 'border-orange-100 dark:border-orange-900/30' : 'border-gray-50 dark:border-zinc-900'}`} onClick={handlePostClick}>
