...
export const PostDetailsModal: React.FC<PostDetailsModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300" onClick={onClose}>
      <div className="bg-white dark:bg-[#0a0a0a] w-full max-w-3xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 border border-white dark:border-zinc-800" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          <div className="p-8 pb-4">
            ...
            <div className="text-gray-800 dark:text-gray-200 leading-relaxed whitespace-pre-wrap text-base font-medium mb-6">{post.content}</div>

            {post.imageUrl && (
              <div className="mb-6 rounded-3xl overflow-hidden border border-slate-100 dark:border-zinc-800 shadow-sm bg-slate-50 dark:bg-zinc-900"><img src={post.imageUrl} className="w-full h-auto object-cover max-h-[600px]" alt="" /></div>
            )}
              ...
              <div className="space-y-6">
                {post.commentsList.map((comment) => (
                  <div key={comment.id} className="flex space-x-4 animate-in fade-in slide-in-from-bottom-2">
                    <img src={comment.authorAvatar} className="w-10 h-10 rounded-xl object-cover" alt="" />
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1"><span className="text-sm font-bold text-slate-900 dark:text-white">{comment.authorName}</span><span className="text-[10px] text-slate-400 font-bold">{timeAgo(comment.timestamp)}</span></div>
        ...
        <div className="p-6 bg-white dark:bg-[#0a0a0a] border-t border-slate-50 dark:border-zinc-900">
          <form onSubmit={handleSubmit} className="flex items-center space-x-3 bg-slate-50 dark:bg-zinc-900 rounded-2xl p-2 border border-slate-100 dark:border-zinc-800 focus-within:ring-4 focus-within:ring-blue-50 transition-all">
            ...
            <button type="submit" disabled={!text.trim()} className="bg-blue-600 text-white p-3 rounded-xl shadow-lg hover:bg-blue-700 disabled:opacity-30 transform active:scale-90"><Send size={18} /></button>
          </form>
        </div>
      </div>
    </div>
