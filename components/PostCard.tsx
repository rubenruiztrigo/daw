    ...
    <>
      ...
      >
        ...
        <div className="mt-2 flex-1 min-w-0 px-2 sm:px-6">
          ...
          <div className="flex items-center justify-between w-full text-gray-500 mt-2">
            {onVote && isNews ? (
              <div className="flex items-center bg-slate-50 dark:bg-zinc-900/50 rounded-xl p-0.5 border border-slate-100 dark:border-zinc-800" onClick={(e) => e.stopPropagation()}>
                <button 
                ...
                <span className={`px-3 font-black text-xs min-w-[2.5rem] text-center ${
                  (post.upvotes || post.likes || 0) > 0 ? 'text-emerald-600' : (post.upvotes || post.likes || 0) < 0 ? 'text-red-600' : 'text-slate-900 dark:text-white'
                }`}>
            ...
            {!isNews && (
              ...
              >
                <div className={`p-2 rounded-full transition-all ${post.userReposted ? 'bg-emerald-50 dark:bg-emerald-900/20' : 'group-hover/btn:bg-emerald-50 dark:group-hover/btn:bg-pink-900/20'}`}>
